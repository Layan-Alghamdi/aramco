...
// Define Template 1 first and make it immutable
const templateOneDefinition = {
...
// Freeze Template 1 to make it immutable
export const TEMPLATE_ONE = deepFreeze(templateOneDefinition);

// All templates array - Template 1 is first and immutable
...
// Clone function that ensures Template 1 is never mutated - always returns a deep copy
export function cloneTemplateSlides(templateId) {
  const template = templateMap[templateId];
  if (!template) return [];

  const cloneObject = (obj) => {
    // Deep clone using JSON to ensure no references to original
  ...
  // Deep clone the entire template structure to ensure Template 1 is never mutated
...
export const slideTemplates = templates;
export const slideTemplateMap = templateMap;
